...
class Tweet: NSManagedObject {
    class func findOrCreateTweet(matching twitterInfo: Twitter.Tweet, in context: NSManagedObjectContext) throws -> Tweet {
        ...
        var mentions = twitterInfo.hashtags
        mentions.append(contentsOf: twitterInfo.userMentions)
        tweet.mentions = try? TwitterMention.findOrCreateTwitterMention(matching: mentions, in: context)
